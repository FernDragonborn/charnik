...
- **ARCH-4 [~]** — розмір-токени не енфорсяться. **Оцінено (правило готове, але блоковане на
  дизайн-рішенні):** `declaration-property-unit-disallowed-list` на `font-size`/padding/margin/gap =
  **753 warning-и** (font-size 21, spacing 523). Проблема: warn-only на 753 = шум, який привчає
  ігнорити stylelint (net-negative); а error-only неможливий без міграції. Але **px-значення НЕ
  лягають на існуючу шкалу** (`font-size:30/22/34/26/18px` — токенів `--font-size-*` = 28/20/44 нема
  точних), тож міграція = або округлення (візуальна зміна → потребує screenshot-верифікації), або
  розширення type-scale (дизайн-рішення). Обидва — рішення користувача, не сліпий sweep.
  **Рекомендація:** (а) вирішити долю display-шрифтів (додати `--font-size-*` для 22/26/30/34 АБО
  округлити до найближчих), (б) тоді ввімкнути правило на `font-size` як ERROR (21 сайт, чисто);
  spacing (523) — окремий поетапний прохід із візуальною верифікацією. Правило НЕ закомічено.
