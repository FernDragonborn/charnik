 ...
 *
 * A step is a snapshot of the plain data the builder holds — the same `$state.snapshot` the autosave
 * and the trial-derive already take. Nothing here knows what a class or a spell is, which is why a
 * new field on `DraftState` is undoable the moment it exists.
 *
 ...
 * One undoable state.
 *
 * The class stash travels with the draft because the two are one fact: the stash holds what a class
 * owned before it was switched away from, so an undo that moved only the draft left a cache
 * describing a swap that no longer happened — and taking that class again handed back picks the
 * player had just taken back. The autosave already writes the pair as one record.
 */
export interface DraftStep {
	draft: DraftState;
	classPicks: [string, ClassScopedPicks][];
}
 ...
 */
const freeze = (step: DraftStep): DraftStep => structuredClone($state.snapshot(step));
