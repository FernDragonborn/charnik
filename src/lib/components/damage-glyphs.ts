/*
 * The damage-type glyphs — Lucide (ISC) path data, carried here rather than as thirteen icon files.
 * A plain module rather than the component's own script block because the KEY LIST is data two
 * places need: `DamageIcon` draws them, and the roller's suggestion menu offers them as the damage
 * types it knows by name. One list, so a type can never be drawable and unofferable.
 *
 * Line art in `currentColor`, deliberately NOT colour coded: a roll's colour belongs to what the DIE
 * did (nat 20 gold, nat 1 red), and a second colour axis for the type would fight it.
 * Lucide's `<circle>`s are written as two-arc paths, so every glyph is a flat string[].
 */
...
/** The damage types the app can draw. Content may name any type it likes — this is the set that
 *  collapses to an icon, and the set the roller menu offers by name. */
export const DAMAGE_TYPES: string[] = Object.keys(GLYPHS);

/** The glyph paths for a type, or an empty array for one the app has never heard of (homebrew is
 *  free to invent types; the pill then keeps the WORD instead of collapsing to an icon). */
export const damageGlyph = (type: string): string[] => GLYPHS[type.trim().toLowerCase()] ?? [];
